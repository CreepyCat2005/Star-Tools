...
namespace Star_Citizen_Pfusch
{
    internal class CookieMonster
    {
        ...
        {
            string cookiePath = Config.BrowserType.GetCookiePath();
            List<Cookie> data = new List<Cookie>();
            if (File.Exists(cookiePath))
            {
                try
                {
                    using (var conn = new SQLiteConnection($"Data Source={cookiePath}"))
                    using (var cmd = conn.CreateCommand())
                    {
                        cmd.CommandText = $"SELECT name,encrypted_value,host_key FROM cookies WHERE host_key LIKE '{hostname}'";
                        byte[] key = AesGcm256.GetKey(Config.BrowserType);
        ...
        class AesGcm256
        {
            public static byte[] GetKey(BrowserEnum browser)
            {
                string v = File.ReadAllText(browser.GetLocalStatePath());
